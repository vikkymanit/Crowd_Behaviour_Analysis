...
%v3 contains the pdf values.

X=load ('avg_vel_v3.txt')
Y=load ('directional_dist_v3.txt')
Z=load ('trajectory_mean_v3.txt')
X=[X(1:2:end)' X(2:2:end)'];
Y=[Y(1:2:end)' Y(2:2:end)'];
Z=[Z(1:2:end)' Z(2:2:end)'];
obj=gmdistribution.fit([X,Y,Z],3);
A=load ('avg_vel_v3an.txt')
B=load ('directional_dist_v3an.txt')
C=load ('trajectory_mean_v3an.txt')
